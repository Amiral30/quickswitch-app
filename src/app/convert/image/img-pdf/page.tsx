...
export default function ImageToPdf() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      saveToHistory(file.name, 'Convertisseur', 'Image ➔ PDF', 'success')
    } catch (err) {
      setError('Erreur lors de la conversion')
      console.error(err)
      saveToHistory(file.name, 'Convertisseur', 'Image ➔ PDF', 'error')
    } finally {
      setConverting(false)
    }
  }
      ...
      <div className="max-w-md w-full glass-premium p-8 rounded-2xl shadow-xl flex flex-col gap-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Générez des pages PDF haute résolution à partir de vos photos.
          </p>
        </div>
