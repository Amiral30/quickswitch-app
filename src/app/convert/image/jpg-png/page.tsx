  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
          ...
          try {
            ...
            canvas.toBlob(blob => {
              if (blob) {
                ...
                reject(new Error("La compression Canvas en PNG a échoué."))
              }
            }, 'image/png')
          } catch (e) {
            reject(e)
        ...
        img.onerror = () => reject(new Error("Impossible de charger l'image JPG."))
        img.src = imageUrl
      })

      saveToHistory(file.name, 'Convertisseur', 'JPG ➔ PNG', 'success')
    } catch (err) {
      setError('Erreur lors de la conversion. Veuillez réutiliser un autre fichier.')
      console.error(err)
      saveToHistory(file.name, 'Convertisseur', 'JPG ➔ PNG', 'error')
    } finally {
      setConverting(false)
    }
  }
      ...
      <div className="max-w-md w-full glass-premium p-8 rounded-2xl shadow-xl flex flex-col gap-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Générez des images PNG sans perte de qualité à partir de vos JPEG.
          </p>
        </div>
