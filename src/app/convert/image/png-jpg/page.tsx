      ...
      await new Promise<void>((resolve, reject) => {
        img.onload = () => {
          try {
            ...
            // Set white background for transparent pixels in PNG
            ...
            canvas.toBlob(blob => {
              if (blob) {
                ...
                reject(new Error("La compression HTML Canvas a échoué."))
              }
            }, 'image/jpeg', 0.95)
          } catch (e) {
            reject(e)
        ...
        img.onerror = () => reject(new Error("Impossible d'importer le fichier image."))
        img.src = imageUrl
      })

      saveToHistory(file.name, 'Convertisseur', 'PNG ➔ JPG', 'success')
    } catch (err) {
      setError('Erreur durant la conversion. Essayez un autre fichier.')
      console.error(err)
      saveToHistory(file.name, 'Convertisseur', 'PNG ➔ JPG', 'error')
    } finally {
      setConverting(false)
    }
  }
      ...
      <div className="max-w-md w-full glass-premium p-8 rounded-2xl shadow-xl flex flex-col gap-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Générez des images JPEG à arrière-plan blanc optimisé.
          </p>
        </div>
